...
8. 매일 아침 손으로 스크립트를 돌리는 건 결국 안 하게 된다. **장 시작 전에 데이터가 알아서 준비돼 있어야** 브리핑을 바로 받을 수 있다. 마침 KRX 가 당일 데이터를 오전 8시부터 제공하므로, **Windows 작업 스케줄러로 매 영업일 08:05 에 무인 실행**되도록 자동화했다(`run_morning.ps1` + `register_krx_task.bat`). 실행이 끝나면 `results/` 의 JSON 을 AI 가 읽어 브리핑을 작성한다.

자세한 의사결정 기록은 아래 [데이터 접근 방식](#데이터-접근-방식-의사결정-기록-2026-06) 참조.
...
매 영업일 아침, 사람 개입 없이 위 두 산출물을 자동 생성.

- `register_krx_task.bat` 로 Windows 작업 스케줄러에 **월~금 08:05** 실행 등록 (1회)
- 매일 `run_morning.ps1` 이 전용 venv 로 브리핑 → 스크리너 순서로 실행, 결과를 `results/` 에 갱신
- **08:05 근거**: KRX OpenAPI 가 당일 데이터를 오전 8시부터 제공 → 여유를 둬 5분 뒤 실행
- 경로는 스크립트 자기 위치를 기준으로 계산해, 폴더를 어디 두든 동작 (자세한 설정·옵션은 [자동화](#자동화-windows-작업-스케줄러) 참조)
...
> 산출 JSON 은 스크립트 폴더 아래 **`results/`** 에 기록된다(각 스크립트의 `OUT_DIR`, 폴더 없으면 자동 생성). 출력 위치를 바꾸려면 해당 상수를 수정. 산출물에는 보유종목·평가손익 등 개인 정보가 담길 수 있어 git 에서 제외된다.
...
> KRX OpenAPI 인증키는 **최대 12개월** 이용 후 만료되며, **만료 30일 전부터 갱신 신청** 가능.
...
| 패키지 | 용도 |
|--------|------|
| `pandas` / `numpy` | 시세·재무 데이터 처리, 팩터 계산 |
| `requests` | KRX OpenAPI / KRX 로그인 / Naver / DART HTTP 호출 |
| `python-dotenv` | `.env` 에서 자격증명·인증키 로드 |
| `yfinance` | NVDA(해외) 시세 — 브리핑 전용, 미설치 시 해당 항목만 생략 |

> 그 외는 모두 파이썬 표준 라이브러리(`os`/`json`/`datetime`/`io`/`zipfile`/`xml.etree` 등)라 별도 설치 불필요.
...
→ **9종목 중 6개 ✅ ±7% 이내**(단순 사업구조), **3개 ⚠️**(삼성전자·삼성생명·현대차 — 지주/소수지분/보험·TTM 차이) ±10~22% 잔차. 가치 팩터는 상대순위 정규화라 스크리너 랭킹 영향은 미미.
